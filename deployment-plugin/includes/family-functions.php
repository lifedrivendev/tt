...
/**
 * Generates a JSON file containing all published family posts.
 ...
 */
function generate_family_data_json_file($dir)
{
    $args = array(
        'post_type' => 'family',
        'posts_per_page' => -1,
    ...
    if (!file_exists($dir) && !wp_mkdir_p($dir)) {
        error_log("Family JSON: Unable to create directory: " . $dir);
        return false;
    }
    $filename = trailingslashit($dir) . 'families.json';
    if (file_put_contents($filename, data: $json) === false) {
        $error = error_get_last();
        error_log("Family JSON: Failed to write file ($filename): " . print_r($error, true));
        return false;
    }
    error_log("Family JSON file generated successfully at: " . $filename);
    return $json;
}
