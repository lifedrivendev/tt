...
/**
 * Generates a JSON file containing all published artist posts.
 ...
 */
function generate_artist_data_json_file($dir)
{
    $args = array(
        'post_type' => 'artist',
        'posts_per_page' => -1,
    ...
    if (!file_exists($dir) && !wp_mkdir_p($dir)) {
        error_log("Artist JSON: Unable to create directory: " . $dir);
        return false;
    }
    $filename = trailingslashit($dir) . 'artists.json';
    if (file_put_contents($filename, data: $json) === false) {
        $error = error_get_last();
        error_log("Artist JSON: Failed to write file ($filename): " . print_r($error, true));
        return false;
    }
    error_log("Artist JSON file generated successfully at: " . $filename);
    return $json;
}
