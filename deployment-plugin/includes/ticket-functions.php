...
/**
 * Generates a JSON file containing all published ticket posts with full ACF details.
 ...
 */
function generate_ticket_data_json_file($dir)
{
    $args = array(
        'post_type' => 'ticket',
        'posts_per_page' => -1,
    ...
    if (!file_exists($dir) && !wp_mkdir_p($dir)) {
        error_log("Ticket JSON: Unable to create directory: " . $dir);
        return false;
    }

    $filename = trailingslashit($dir) . 'tickets.json';
    if (file_put_contents($filename, $json) === false) {
        $error = error_get_last();
        error_log("Ticket JSON: Failed to write file ($filename): " . print_r($error, true));
        return false;
    }

    error_log("Ticket JSON file generated successfully at: " . $filename);
    return $json;
}
